...
export default function ServerOverviewPage() {
  ...
  const stats = [
    { label: "CPU", value: `${metrics.cpu[metrics.cpu.length - 1]?.value || server.cpu}%`, icon: Cpu, color: "from-ryzen-500 to-emerald-600", data: metrics.cpu },
    ...
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="space-y-6">
        ...
        {stats.map((s, idx) => (
          <motion.div key={s.label} initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: idx * 0.05 }}
            ...
            <div className="flex items-center gap-3 mb-2">
              <div className={`rounded-lg p-2 bg-gradient-to-r ${s.color} bg-opacity-10`}>
              ...
              <div className="flex-1 min-w-0">
                ...
                {s.data.length > 1 && (
                  <div className="absolute right-2 top-2 opacity-20 group-hover:opacity-40 transition-opacity">
                    <MiniSparkline data={s.data} color={s.color.includes("ryzen") ? "#34d399" : s.color.includes("blue") ? "#3b82f6" : s.color.includes("violet") ? "#8b5cf6" : "#f59e0b"} />
                  </div>
                )}
              </div>
            </div>
            ...
            {s.data.length > 1 && (
              <div className="mt-2 h-1 w-full rounded-full bg-slate-800 overflow-hidden">
                <motion.div className="h-full rounded-full" style={{
                  background: `linear-gradient(90deg, ${s.color.includes("ryzen") ? "#10b981" : s.color.includes("blue") ? "#3b82f6" : s.color.includes("violet") ? "#8b5cf6" : "#f59e0b"}, ${s.color.includes("ryzen") ? "#34d399" : s.color.includes("blue") ? "#06b6d4" : s.color.includes("violet") ? "#a855f7" : "#f97316"})`,
      ...
      <div className="grid gap-4 grid-cols-2 lg:grid-cols-4">
        {[metrics.cpu, metrics.ram, metrics.disk, metrics.network].map((data, i) => {
          ...
          const max = Math.max(...data.map(d => d.value), 1);
          return (
            <div key={labels[i]} className="card">
                ...
                {data.map((d, idx) => (
                  <div key={idx} className="flex-1 flex flex-col justify-end">
                    <motion.div
                      ...
                      style={{ backgroundColor: colors[i], opacity: 0.3 + (idx / data.length) * 0.5 }}
                    />
                  </div>
                ))}
              </div>
          ...
          {activity.length === 0 ? (
            ...
            <div className="space-y-2 max-h-64 overflow-y-auto">
              {activity.slice(0, 15).map((a: any) => (
                <div key={a.id} className="flex items-center justify-between rounded-lg bg-slate-800/20 px-3 py-2">
                  <div className="flex items-center gap-2 min-w-0">
                    <div className={`h-1.5 w-1.5 rounded-full flex-shrink-0 ${a.status === "SUCCESS" ? "bg-ryzen-400" : a.status === "WARNING" ? "bg-yellow-400" : "bg-red-400"}`} />
