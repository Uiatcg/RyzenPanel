...
export async function POST(req: Request) {
  ...
  try {
    ...
    return NextResponse.json({
      ...
      installCommand: `bash <(curl -s https://raw.githubusercontent.com/anomalyco/ryzenpanel/main/install.sh) --wings --panel-url="${process.env.NEXT_PUBLIC_BASE_URL || "https://YOUR_PANEL_DOMAIN"}" --node-uuid="${node.uuid}" --node-secret="${node.nodeSecret}" --daemon-key="${node.daemonKey}" --fqdn="${fqdn}"`,
    });
  } catch (error) {
